		...
		<div class="header__body">
			...
			<div class="header__search">
				<form action="{{route('almacen.all')}}">
					<input class="header__search-input" id="inputSearchDesktop" type="search" autocomplete="off" name="search" placeholder="Buscar productos, marcas y más...">
				</form>
				...
				<div class="header__searchItemsMain" id="searchedItems">
					<div class="header__searchItems" id="searchedItemsContainer">
					</div>
				</div>
			</div>
...
(() => {
	document.addEventListener('DOMContentLoaded', () => {
		function showOrHideSearchedItems(value){
			const searchedContainer = document.getElementById('searchedItems');
		...
		function addProductSearched(items){
			const container = document.getElementById('searchedItemsContainer');
			container.innerHTML = ''
			if(items.length){
				items.forEach(({ slug ,image, title, sku, available_stock, packaging: { packaging }}) => {
					template = `
						<div>
							<a href="/producto/${slug}" class="header__searchItems-elements">
								<div class="header__searchItems-image">
									<img src="/storage/${image}" alt="Imagen Producto">
								</div>
								<p class="header__searchItems-content">${title} - SKU: ${sku} <span>- ${available_stock} productos por ${packaging}</span></p>
							</a>
						</div>
					`
		...
		//INPUT SEARCH
		const inputSearch = document.getElementById('inputSearchDesktop');
